...
from grover.data import MolCollator, MolKGNNCollator
...
from grover.util.utils import build_optimizer, build_lr_scheduler, makedirs, load_checkpoint, get_loss_func, \
    save_checkpoint, build_model, build_model_kgnn, load_checkpoint_kgnn
from grover.util.utils import get_class_sizes, get_data, split_data, get_task_names, get_data_kgnn, split_data_kgnn
from task.predict_kgnn import predict, evaluate, evaluate_predictions
...
def train(epoch, model, data, loss_func, optimizer, scheduler,
    ...
    mol_collator = MolKGNNCollator(shared_dict=shared_dict, args=args)
    num_workers = 4
    ...
    for _, item in tqdm(enumerate(mol_loader)):
        ...
        # Run model
        model.zero_grad()
        preds = model(m_batch, k_batch, features_batch)
        # print("preds: ", preds)
...
def run_training(args: Namespace, time_start, logger: Logger = None) -> List[float]:
    ...
    for model_idx in range(args.ensemble_size):
        ...
        if args.checkpoint_paths is not None:
            ...
            model = load_checkpoint_kgnn(args.checkpoint_paths[cur_model], current_args=args, logger=logger)
        else:
            debug(f'Building model {model_idx}')
            model = build_model_kgnn(model_idx=model_idx, args=args)
        ...
        loss_func = get_loss_func(args, model.model_mgnn)
        ...
        # Bulid data_loader
        shuffle = True        
        mol_collator = MolKGNNCollator(shared_dict={}, args=args)
        ...
        model = load_checkpoint_kgnn(os.path.join(save_dir, 'model.pt'), cuda=args.cuda, logger=logger)
...
def load_data(args, debug, logger):
    ...
    data = get_data_kgnn(path=args.data_path, args=args, logger=logger)
    ...
    if args.separate_test_path:
        test_data = get_data_kgnn(path=args.separate_test_path, args=args,
                             features_path=args.separate_test_features_path, logger=logger)
    if args.separate_val_path:
        val_data = get_data_kgnn(path=args.separate_val_path, args=args,
                            features_path=args.separate_val_features_path, logger=logger)
    if args.separate_val_path and args.separate_test_path:
        train_data = data
    elif args.separate_val_path:
        train_data, _, test_data = split_data_kgnn(data=data, split_type=args.split_type,
                                              sizes=(0.8, 0.2, 0.0), seed=args.seed, args=args, logger=logger)
    elif args.separate_test_path:
        train_data, val_data, _ = split_data_kgnn(data=data, split_type=args.split_type,
                                             sizes=(0.8, 0.2, 0.0), seed=args.seed, args=args, logger=logger)
    else:
        train_data, val_data, test_data = split_data_kgnn(data=data, split_type=args.split_type,
